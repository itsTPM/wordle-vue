...
<template>
  <Toaster position="top-center" :expand="true" :duration="1500" :theme="themeStore.currentTheme" />
  ...
  <header
    ...
    <ul class="absolute bottom-1/2 translate-y-1/2 right-2 flex xs:gap-2">
      <li class="h-12">
        <Button
          ...
          :aria-label="themeStore.currentTheme === 'light' ? 'Switch to dark theme' : 'Switch to light theme'"
          @click="themeStore.toggleTheme()">
          <IconMoon
            class="w-8 aspect-square absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"
            stroke-width="2"
            aria-hidden="true"
            v-if="themeStore.currentTheme === 'light'"></IconMoon>
          <IconSun
            class="w-8 aspect-square absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"
            stroke-width="2"
            aria-hidden="true"
        ...
        <Dialog v-model:open="guideDialogOpen">
          ...
          <template #content>
            <DialogContent>
              ...
              <DialogDescription class="text-center text-sm pt-0 text-black/75 dark:text-white/75">
                How to play
              </DialogDescription>
              <div class="flex flex-col gap-1">
                <p>In Wordle game you need to guess word with only 6 tries.</p>
                <p class="border-l-4 pl-4 text-black/75 dark:text-white/75">
                  Each guess should be a valid 5-letter word from English dictionary.
                </p>
                <hr class="my-4" />
                <p class="font-medium">Highlighting</p>
                <ul>
                  <li>
                    <span class="text-green">Green</span> means that the letter is in the word, and located in correct
                    place.
                  </li>
                  <li>
                    <span class="text-yellow">Yellow</span> means that the letter is in the word, but is located in a
                    different place.
                  </li>
                  <li><span class="text-gray">Gray</span> means that the letter is not presented in the word.</li>
                </ul>
                <hr class="my-4" />
                <p class="font-medium">Change game mode</p>
                <p>You can select between 'Word of the day' and 'Random word' modes.</p>
                <div
                  class="uppercase px-6 py-2 bg-secondary text-secondary-foreground font-bold text-xs text-center max-w-48 select-none">
                  <template v-if="gameStore.currentGameMode === 'wordOfTheDay'"> Word of the day </template>
                  <template v-if="gameStore.currentGameMode === 'random'"> Random word </template>
                  <template v-else-if="gameStore.currentGameMode === 'custom'"> Custom word </template>
                </div>
          ...
          <template #content>
            <DialogContent aria-describedby="undefined">
              ...
              <ul class="flex flex-col gap-6 py-6">
                <li class="flex gap-2" v-for="(setting, keyName) in settingsStore.$state">
                  <Switch :id="keyName" v-model="setting.value" v-if="setting.type === 'switch'"></Switch>
                  <select
                    ...
                    <option v-for="(option, idx) in setting.options" :key="idx" :value="option.value">
                      {{ option.title }}
                    </option>
                  </select>
            ...
            <DialogContent aria-describedby="undefined">
              ...
              <template v-for="(gameMode, _, idx) in statisticsStore.$state">
                <div class="flex flex-col gap-1">
                  ...
                  <p v-else class="text-black/50 dark:text-white/50">No games played in this mode yet</p>
                </div>
                <hr class="my-4" v-if="idx != Object.keys(statisticsStore.$state).length - 1" />
              </template>
              <Button
    ...
    <WordleVueLogo class="fill-foreground max-h-xs:h-5 h-6" aria-hidden="true"></WordleVueLogo>

    <h1 class="sr-only">Wordle Vue game</h1>

    <p class="uppercase tracking-wide font-extrabold text-xs text-black/25 dark:text-white/25 -mt-1 select-none">
      in development
    </p>
  </header>

  <main class="flex flex-col max-h-xs:gap-2 gap-4 items-center w-full pt-2 min-h-xs:py-4 relative">
        ...
        <template #content>
          <DialogContent>
            ...
            <template v-if="customsStore.customLink === ''">
              <DialogDescription> Enter any word and get a special link to share with your friend </DialogDescription>
              <form
            ...
            <template v-else>
              ...
              <div class="flex flex-col xs:grid grid-rows-1 grid-cols-4 gap-2 w-full">
                <Input
                  type="text"
                  disabled
                  name="customLink"
    ...
    <div class="w-72 max-w-80 max-h-96 xs:w-80 xs:h-96 flex flex-col max-h-xs:gap-1 select-none gap-2">
