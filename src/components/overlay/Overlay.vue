...
<template>
  <div
    ...
    <p class="text-3xl text-center font-semibold text-foreground">
      <template v-if="gameStore.isGameWon"> 🎉 Congratulations! You won! </template>
      <template v-else-if="gameStore.isGameLost">
        😭 You lost! The word was
        <span class="underline underline-offset-4 font-extrabold">{{ gameStore.word }}</span>
      </template>
    </p>
    ...
    <ul class="flex flex-col gap-2 mt-2">
      <li>
        <OverlayButton @click="gameStore.changeGameMode('random')"> Play in Random mode </OverlayButton>
      </li>
      <li v-if="gameStore.currentGameMode !== 'wordOfTheDay'">
        <OverlayButton @click="gameStore.changeGameMode('wordOfTheDay')"> Play in Word of the day mode </OverlayButton>
      </li>
    </ul>
  </div>
</template>
